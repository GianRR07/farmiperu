...
// Obtener dirección textual desde coordenadas (OpenStreetMap)
...
export default function DireccionMap({ onDireccionSeleccionada }) {
  const [principal, setPrincipal] = useState(null); // Dirección fija actual
  const [temporal, setTemporal] = useState(null); // Nueva selección temporal
  const [center, setCenter] = useState([-12.0464, -77.0428]);
  const [cargando, setCargando] = useState(true);
  const inputRef = useRef(null);

  // Detectar ubicación inicial (actual)
  useEffect(() => {
  ...
  const handleBuscar = async () => {
    ...
    if (resultado) {
      setTemporal(resultado); // marcador temporal
  ...
  // Guardar dirección temporal como principal
  ...
  // Ir a la dirección principal (centrar mapa)
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Mostrar dirección seleccionada (temporal) */}
      {temporal && (
