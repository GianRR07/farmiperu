...
export default function CarritoCompras({ isOpen, onClose }) {
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full w-80 bg-white shadow-lg transform transition-transform duration-300 z-50 ${open ? 'translate-x-0' : 'translate-x-full'
        }`}
    >
          ...
          items.map(item => (
            <div key={item.id} className="flex gap-3 items-center border-b pb-3">
              ...
              <div className="flex-1">
                ...
                {item.presentacion && <p className="text-xs text-gray-500">{item.presentacion}</p>}
                <p className="text-sm text-green-700 font-semibold">S/ {Number(item.precio).toFixed(2)}</p>
